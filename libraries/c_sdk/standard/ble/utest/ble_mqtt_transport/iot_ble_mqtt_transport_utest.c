...
MQTTStatus_t basicConnectCallback( const MQTTConnectInfo_t * const pConnectInfo,
                                          ...
                                          size_t * const pPacketSize, int num_calls )
{
    TEST_ASSERT( pConnectInfo->cleanSession );
    TEST_ASSERT( 0 == strncmp(pConnectInfo->pClientIdentifier, "Connectbasic", 12));
    TEST_ASSERT( pConnectInfo->clientIdentifierLength == 12U);
    TEST_ASSERT( pConnectInfo->keepAliveSeconds == 10U);
...
{
    uint8_t MQTTPacket[] = { 0x10, 0x18, 0x00, 0x04, 0x4d, 0x51, 0x54, 0x54, 0x04, 0x02,
    ...
    TEST_ASSERT( bytesSent == packetSize );
}
...
{
    TEST_ASSERT( pConnectInfo->cleanSession );
    TEST_ASSERT( 0 == strncmp(pConnectInfo->pClientIdentifier, "Connectcredentials", 18));
    TEST_ASSERT( pConnectInfo->clientIdentifierLength == 18U);
    TEST_ASSERT( pConnectInfo->keepAliveSeconds == 10U);
    TEST_ASSERT( 0 == strncmp(pConnectInfo->pUserName, "abc", 3U ));
    TEST_ASSERT( 0 == strncmp(pConnectInfo->pPassword, "012", 3U ));
    TEST_ASSERT( pConnectInfo->passwordLength == 3U);
    TEST_ASSERT( pConnectInfo->userNameLength == 3U);
...
{
    uint8_t MQTTPacket[] = { 0x10, 0x2a, 0x00, 0x04, 0x4d, 0x51, 0x54, 0x54, 0x04, 0xc2,
...
{
    uint8_t MQTTPacket[] = { 0x10, 0x34, 0x00, 0x04, 0x4d, 0x51, 0x54, 0x54, 0x04, 0xce,
                             0x00, 0x0a, 0x00, 0x12, 0x43, 0x6f, 0x6e, 0x6e, 0x65, 0x63,
                             0x74, 0x63, 0x72, 0x65, 0x64, 0x65, 0x6e, 0x74, 0x69, 0x61,
...
{
    uint8_t MQTTPacket[] = { 0x10, 0x33, 0x00, 0x04, 0x4d, 0x51, 0x54, 0x54, 0x04, 0xce,
                             0x00, 0x0a, 0x00, 0x12, 0x43, 0x6f, 0x6e, 0x6e, 0x65, 0x63,
                             0x74, 0x63, 0x72, 0x65, 0x64, 0x65, 0x6e, 0x74, 0x69, 0x61,
...
{
    uint8_t MQTTPacket1[] = { 0x10, 0x2a, 0x00, 0x04, 0x4d, 0x51, 0x54, 0x54, 0x04, 0xc2,
                              0x00, 0x0a, 0x00, 0x12, 0x43, 0x6f, 0x6e, 0x6e, 0x65, 0x63,
                              0x74, 0x63, 0x72, 0x65, 0x64, 0x65, 0x6e, 0x74, 0x69, 0x61,
...
void test_IotBleMqttTransportSend_ConnectBadParameters( void )
{
    ...
    uint8_t MQTTPacketBadProtocol[] = { 0x10, 0x18, 0x00, 0x04, 0x4d, 0x51, 0x54, 0x54, 0x06, 0x01,
                                        0x00, 0x00 };
    bytesSent = ( size_t ) IotBleMqttTransportSend( &context, ( void * ) MQTTPacketBadProtocol, packetSize );
    TEST_ASSERT( bytesSent == 0 );

    /* Test for too high will Qos */
    uint8_t MQTTPacketWillQos[] = { 0x10, 0x18, 0x00, 0x04, 0x4d, 0x51, 0x54, 0x54, 0x04, 0x18,
                                        0x00, 0x00 };
    packetSize = 12U;
    ...
    uint8_t MQTTPacketConnectFlag[] = { 0x10, 0x18, 0x00, 0x04, 0x4d, 0x51, 0x54, 0x54, 0x04, 0x11,
                                        0x00, 0x00 };
    packetSize = 12U;
    ...
    uint8_t MQTTPacketNoID[] = { 0x10, 0x18, 0x00, 0x04, 0x4d, 0x51, 0x54, 0x54, 0x04, 0x02,
                                 0x00, 0x00, 0x00, 0x00 };
    packetSize = 12U;
...
void test_IotBleMqttTransportSend_PublishBasic( void )
{
    ...
    size_t bytesSent = 0;
    uint8_t MQTTPacket[] = { 0x30, 0x24, 0x00, 0x11, 0x69, 0x6f, 0x74, 0x64, 0x65, 0x6d,
 ...
 * @details Mock out a non MQTTSuccess response from deserialize, and make sure nothing sends
 */
...
MQTTStatus_t basicSubscribeCallback( const MQTTSubscribeInfo_t * const pSubscriptionList,
                                            ...
                                            uint16_t * const pPacketIdentifier, int num_calls )
{
    TEST_ASSERT( subscriptionCount == 1 );
    TEST_ASSERT( pSubscriptionList->qos == MQTTQoS0 );
    TEST_ASSERT( pSubscriptionList->topicFilterLength == 0x0e );
    TEST_ASSERT( 0 == strncmp(pSubscriptionList->pTopicFilter, "basicSubscribe", 0x0e));
...
{
    size_t bytesSent = 0;
    uint8_t MQTTPacket[] = { 0x82, 0x13, 0x00, 0x01, 0x00, 0x0e, 0x62, 0x61, 0x73, 0x69,
...
MQTTStatus_t multiSubscribeCallback( const MQTTSubscribeInfo_t * const pSubscriptionList,
                                            ...
                                            uint16_t * const pPacketIdentifier, int num_calls )
{
    TEST_ASSERT( subscriptionCount == 2 );
    TEST_ASSERT( pSubscriptionList[0].qos == MQTTQoS0 );
    TEST_ASSERT( pSubscriptionList[0].topicFilterLength == 0x0f );
    TEST_ASSERT( 0 == strncmp(pSubscriptionList[0].pTopicFilter, "multiSubscribe0", 0x0f));

    TEST_ASSERT( pSubscriptionList[1].qos == MQTTQoS0 );
    TEST_ASSERT( pSubscriptionList[1].topicFilterLength == 0x0f );
    TEST_ASSERT( 0 == strncmp(pSubscriptionList[1].pTopicFilter, "multiSubscribe1", 0x0f));
...
{
    size_t bytesSent = 0;
    uint8_t MQTTPacket[] = { 0x82, 0x26, 0x00, 0x3b, 0x00, 0x0f, 0x6d, 0x75, 0x6c, 0x74,
...
{
    TEST_ASSERT( subscriptionCount == 2 );
    TEST_ASSERT( pSubscriptionList[0].qos == MQTTQoS1 );
    TEST_ASSERT( pSubscriptionList[0].topicFilterLength == 0x0f );
    TEST_ASSERT( 0 == strncmp(pSubscriptionList[0].pTopicFilter, "multiSubscribe0", 0x0f));

    TEST_ASSERT( pSubscriptionList[1].qos == MQTTQoS1 );
    TEST_ASSERT( pSubscriptionList[1].topicFilterLength == 0x0f );
    TEST_ASSERT( 0 == strncmp(pSubscriptionList[1].pTopicFilter, "multiSubscribe1", 0x0f));
...
{
    size_t bytesSent = 0;
    uint8_t MQTTPacket[] = { 0xa2, 0x24, 0x00, 0x3b, 0x00, 0x0f, 0x6d, 0x75, 0x6c, 0x74,
                             0x69, 0x53, 0x75, 0x62, 0x73, 0x63, 0x72, 0x69, 0x62, 0x65,
...
void receiveCallback ( IotBleDataTransferChannel_t * pChannel,
                                           ...
                                           size_t * pBufferLength, int num_calls )
{
...
MQTTStatus_t forgePacketIdentifierGood( MQTTPacketInfo_t * pPuback,
                                           uint16_t * packetIdentifier, int num_calls )
{
    *packetIdentifier = 1U;
    return MQTTSuccess;
}
...
size_t checkConnackPacket( StreamBufferHandle_t xStreamBuffer,
						  ...
						  TickType_t xTicksToWait, int num_calls )
{
    uint8_t * bytes = ( uint8_t * ) pvTxData;

    TEST_ASSERT( bytes[0] == MQTT_PACKET_TYPE_CONNACK );
    TEST_ASSERT( bytes[1] == 2U );
    TEST_ASSERT( bytes[2] == 0U );
    TEST_ASSERT( bytes[3] == 0U );
...
void test_IotBleMqttTransportAccept_Connack( void )
{
    ...
    IotBleMqttTransportAcceptData( &context );
}
						  ...
						  TickType_t xTicksToWait, int num_calls )
{
    ...
    TEST_ASSERT( bytes[0] == MQTT_PACKET_TYPE_PUBACK );
    TEST_ASSERT( bytes[1] == 2U );
    TEST_ASSERT( bytes[2] == 0U );
    TEST_ASSERT( bytes[3] == 1U );
...
MQTTStatus_t changePacketSize( const MQTTPublishInfo_t * pPublishInfo,
                                        ...
                                        size_t * pPacketSize, int num_calls )
{
    *pPacketSize = 5U;
    return MQTTSuccess;
}
...
{
    uint8_t buf[] = { 0xa5, 0x61, 0x69, 0x18, 0x3b, 0x61, 0x6b, 0x51, 0x48, 0x65,
						  ...
						  TickType_t xTicksToWait, int num_calls )
{
    ...
    TEST_ASSERT( bytes[0] == MQTT_PACKET_TYPE_SUBACK );
    TEST_ASSERT( bytes[1] == 3U );
    TEST_ASSERT( bytes[2] == 0U );
    TEST_ASSERT( bytes[3] == 1U );
    TEST_ASSERT( bytes[4] == 1U );
						  ...
						  TickType_t xTicksToWait, int num_calls )
{
    ...
    TEST_ASSERT( bytes[0] == MQTT_PACKET_TYPE_UNSUBACK );
    TEST_ASSERT( bytes[1] == 2U );
    TEST_ASSERT( bytes[2] == 0U );
    TEST_ASSERT( bytes[3] == 1U );
...
/**
 * @brief Attempts to accepts an Unsuback packet
 * @details Mock out a non MQTTSuccess response from Deserializer
 *          and make sure nothing is sent to the buffer
 */
						  ...
						  TickType_t xTicksToWait, int num_calls )
{
    ...
    TEST_ASSERT( bytes[0] == MQTT_PACKET_TYPE_PINGRESP );
    TEST_ASSERT( bytes[1] == 0U );
...
void test_IotBleMqttTransportReceive_Bad( void )
{
    ...
    //Test for pubrec being denied
    buf[0] = 0x50;
    IotBleMqttTransportSend( &context, buf, 1);
