...
#define NETWORK_BUFFER_SIZE		1536
	...
	#error The EMAC of SAM4E has fixed-size RX buffers so ZERO_COPY_RX is not possible
#endif
...
static BaseType_t prvSAM_GMAC_Initialise( NetworkInterface_t *pxInterface );
static BaseType_t prvSAM_GMAC_Output( NetworkInterface_t *pxInterface, NetworkBufferDescriptor_t * const pxBuffer, BaseType_t bReleaseAfterSend );
static BaseType_t prvSAM_GMAC_GetPhyLinkStatus( NetworkInterface_t *pxInterface );
...
const TickType_t x5_Seconds = 5000UL;
	...
	return prvSAM_GMAC_GetPhyLinkStatus( NULL );
}
/*-----------------------------------------------------------*/
...
is declared static or global, and that it will remain to exist. */
	...
	pxInterface->pfInitialise		= prvSAM_GMAC_Initialise;
	pxInterface->pfOutput			= prvSAM_GMAC_Output;
	pxInterface->pfGetPhyLinkStatus = prvSAM_GMAC_GetPhyLinkStatus;
...
{
	return pxSAM_GMAC_FillInterfaceDescriptor( xEMACIndex, pxInterface );
}
/*-----------------------------------------------------------*/
...
static BaseType_t prvSAM_GMAC_Output( NetworkInterface_t *pxInterface, NetworkBufferDescriptor_t * const pxDescriptor, BaseType_t bReleaseAfterSend )
{
...
//#if( ipconfigDRIVER_INCLUDED_TX_IP_CHECKSUM == 1 ) && ( ipconfigHAS_TX_CRC_OFFLOADING == 0 )
	...
	ProtocolPacket_t *xProtPacket = (ProtocolPacket_t *)apBuffer;
		...
		if ( xProtPacket->xTCPPacket.xEthernetHeader.usFrameType == ipIPv4_FRAME_TYPE )
		{
			...
			usGenerateProtocolChecksum( ( uint8_t * ) apBuffer, ipconfigNETWORK_MTU, pdTRUE );
		}
	}
