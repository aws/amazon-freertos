...
static cy_rslt_t compute_div(uint32_t input_hz, uint32_t desired_hz, uint32_t divider_bits, const cyhal_clock_tolerance_t *tolerance, uint32_t *div)
{
    ...
    else
    {
        ...
        uint32_t diff;
        uint32_t diff1 = (uint32_t)abs(cyhal_utils_calculate_tolerance(tolerance->type, desired_hz, input_hz / ((uint32_t)div1 + 1)));
        uint32_t diff2 = (uint32_t)abs(cyhal_utils_calculate_tolerance(tolerance->type, desired_hz, input_hz / ((uint32_t)div2 + 1)));
        if (diff1 < diff2)
        {
            diff = diff1;
            *div = div1;
...
cy_rslt_t cyhal_clock_set_frequency(cyhal_clock_t *clock, uint32_t hz, const cyhal_clock_tolerance_t *tolerance)
{
    ...
    switch (clock->block)
    {
        ...
        case CYHAL_CLOCK_BLOCK_PERI:
        {
            ...
            if (CY_RSLT_SUCCESS == rslt)
            {
                if (CYHAL_CLOCK_BLOCK_PERI == clock->block)
                    Cy_SysClk_ClkPeriSetDivider((uint8_t)div);
                else
                    Cy_SysClk_ClkFastSetDivider((uint8_t)div);
        ...
        case CYHAL_CLOCK_BLOCK_SLOW:
        {
            ...
            if (CY_RSLT_SUCCESS == rslt)
            {
                Cy_SysClk_ClkSlowSetDivider((uint8_t)div);
                SystemCoreClockUpdate();
            }
