...
void cyhal_tcpwm_irq_handler(void)
{
    ...
    for (block = 0; block < CY_IP_MXTCPWM_INSTANCES; block++)
    {
        ...
        {
            channel = (uint8_t)(irqn - CYHAL_TCPWM_DATA[block].isr_offset);
            break;
        }
    }
    ...
    if (block < CY_IP_MXTCPWM_INSTANCES)
    {
        ...
        if (tcpwm->callback_data.callback != NULL)
        {
            ...
            (void) (callback) (tcpwm->callback_data.callback_arg, (int32_t)Cy_TCPWM_GetInterruptStatus(blockAddr, (uint32_t)channel));
        }
