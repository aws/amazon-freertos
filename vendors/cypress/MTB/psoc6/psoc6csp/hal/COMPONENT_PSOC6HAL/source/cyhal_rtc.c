...
cy_rslt_t cyhal_rtc_read(cyhal_rtc_t *obj, struct tm *time)
{
    ...
    int year = (int)(dateTime.year + get_rtc_century());
    ...
    time->tm_year = (int)(year - CYHAL_TM_YEAR_BASE);
...
cy_rslt_t cyhal_rtc_write(cyhal_rtc_t *obj, const struct tm *time)
{
    ...
    do {
        ...
        if (rslt == CY_RSLT_SUCCESS)
            set_rtc_century((uint16_t)(time->tm_year) - (uint16_t)(year2digit) + (uint16_t)(CYHAL_TM_YEAR_BASE));
        cyhal_system_critical_section_exit(savedIntrStatus);
        ++retry;
