...
esp_err_t aws_esp_ota_begin( const esp_partition_t * partition,
                             size_t image_size,
...
esp_err_t aws_esp_ota_write( esp_ota_handle_t handle,
                             const void * data,
                             uint32_t offset,
                             size_t size )
{
...
static esp_err_t rewrite_ota_seq( uint32_t seq,
                                  uint8_t sec_id,
    ...
    if( partition->type == ESP_PARTITION_TYPE_APP )
    {
        ...
        else
        {
            ...
            if( find_partition != NULL )
            {
                ...
                #endif /* ifdef CONFIG_APP_ANTI_ROLLBACK */
...
static const esp_partition_t * _esp_get_otadata_partition( uint32_t * offset,
                                                           ota_select * entry,
                                                           bool active_part )
{
    esp_err_t ret;
...
esp_err_t aws_esp_ota_set_boot_flags( uint32_t flags,
                                      bool active_part )
{
...
esp_err_t aws_esp_ota_get_boot_flags( uint32_t * flags,
                                      bool active_part )
{
    const esp_partition_t * part = NULL;
    uint32_t offset;
