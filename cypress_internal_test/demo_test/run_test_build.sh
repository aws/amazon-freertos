...
function compile_make()
{
    ...
    case "$toolchain" in
        GCC_ARM*)
        make build $additional_flags CY_COMPILER_PATH="\"$GCC_DIR\"" CY_BUILD_LOCATION=$build_dir TOOLCHAIN=$toolchain -j4
        ;;
        ARM*)
        make build $additional_flags CY_COMPILER_PATH="\"$ARMCC_DIR\"" CY_BUILD_LOCATION=$build_dir TOOLCHAIN=$toolchain -j4
        ;;
        IAR*)
        make build $additional_flags CY_COMPILER_PATH="\"$IAR_DIR\"" CY_BUILD_LOCATION=$build_dir TOOLCHAIN=$toolchain -j4
        ;;
    esac
    test_error=$?
