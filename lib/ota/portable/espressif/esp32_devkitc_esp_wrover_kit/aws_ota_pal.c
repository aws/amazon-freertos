...
static OTA_Err_t asn1_to_raw_ecdsa(uint8_t *signature, uint16_t sig_len, uint8_t *out_signature)
{
    int ret = 0;
...
extern BaseType_t PKCS11_PAL_ReadFile( char * pcFileName, uint8_t **ppucData, uint32_t *pulDatasize);
u8 * prvPAL_ReadAndAssumeCertificate(const u8 * const pucCertName, uint32_t * const ulSignerCertSize)
{
    u8 *pucSignerCert = NULL;
    if (PKCS11_PAL_ReadFile( (char *)pucCertName, &pucSignerCert, ulSignerCertSize) != pdTRUE) { 
        // use the back up "aws_ota_codesigner_certificate.h" file if the signing credentials haven't been saved in the device
        uint32_t certSize = sizeof(signingcredentialSIGNING_CERTIFICATE_PEM);
        pucSignerCert = pvPortMalloc(certSize);
        if (pucSignerCert != NULL) {
            memcpy(pucSignerCert, (uint8_t*)signingcredentialSIGNING_CERTIFICATE_PEM, certSize);
            *ulSignerCertSize = certSize;
        } else {
...
int16_t prvPAL_WriteBlock(OTA_FileContext_t * const C, uint32_t iOffset, uint8_t *const pacData, uint32_t iBlockSize)
{
    if (_esp_ota_ctx_validate(C)) {
