...
def add_proof_jobs(proof_directory, litani):
    ...
    proof_name = harnesses[0][:-len("_harness.c")]
    goto_binary = str(
        (proof_directory / ("%s_harness.goto" % proof_name)).resolve())
...
def configure_proof_dirs(proof_dirs, counter, litani):
    for proof_dir in proof_dirs:
        print_counter(counter)

        success = add_proof_jobs(proof_dir, litani)
...
def main():
    ...
    configure_proof_dirs(proof_dirs, counter, litani)
